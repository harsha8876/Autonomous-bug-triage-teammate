...
export default function Board({ accent, shadow, board, dragOver, handlers }) {
  return (
    <main style={{ maxWidth: 1180, margin: '0 auto', padding: '28px 22px 90px' }}>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ margin: '0 0 4px', fontSize: 22, fontWeight: 600, letterSpacing: '-0.02em', color: '#2F2C20' }}>Board</h1>
        <p style={{ margin: 0, fontSize: 13.5, color: '#7C795F' }}>Drag cards across columns as triage moves to done.</p>
      </div>
      <div style={{ display: 'flex', gap: 16, overflowX: 'auto', paddingBottom: 10, alignItems: 'flex-start' }}>
        {COL_DEFS.map(([key, name]) => {
          ...
          return (
            <div key={key} style={{ flex: '1 0 248px', minWidth: 248, display: 'flex', flexDirection: 'column', gap: 11 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0 4px' }}>
                <span style={{ fontSize: 13, fontWeight: 600, color: '#46442F' }}>{name}</span>
              ...
              >
                {cards.map((card) => (
                  <HoverDiv
                    ...
                    baseStyle={{ background: '#FEFAE0', border: '1px solid rgba(61,58,46,0.1)', borderRadius: 11, padding: 13, display: 'flex', flexDirection: 'column', gap: 10, cursor: 'grab', boxShadow: shadow, transition: 'box-shadow .15s, border-color .15s' }}
                    hoverStyle={{ borderColor: 'rgba(212,163,115,0.6)' }}
                  >
                    <p style={{ margin: 0, fontSize: 13.5, fontWeight: 500, lineHeight: 1.45, color: '#3A3528' }}>{card.title}</p>
